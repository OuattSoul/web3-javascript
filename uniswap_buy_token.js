...
const web3 = createAlchemyWeb3("https://eth-rinkeby.alchemyapi.io/v2/API_KEY");

const sender_address = "0xe3242ca2b4036f90f42C6D7861af28d06c6161cC";

const uniRouterContractAddress = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D"; 
...
const buyToken = async function(){
    const balance = await web3.eth.getBalance(sender_address);
    const readeable_balance = web3.utils.fromWei(balance, 'ether');
    console.log("Your actual balance is ",readeable_balance, " ETH");

    const contract = new web3.eth.Contract(panABI, uniRouterContractAddress);

    const token_to_buy = '0x93341222b303992e553Bb5A9aaf66BCA9b9E12Cf'; // TOR Token
    
    const wrapped_address = '0xc778417e063141139fce010982780140aa0cd5ab'; // WETH
    ...
    const tx = await contract.methods.swapExactETHForTokens(
        amountIn,
        [wrapped_address, token_to_buy],
        sender_address,
        Math.floor(start / 1000) + 60 * 10
    );
    ...
    console.log(
        `Transaction approved with hash: ${createReceipt.transactionHash}`
    );
}
